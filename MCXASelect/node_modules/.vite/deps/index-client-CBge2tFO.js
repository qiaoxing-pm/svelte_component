import { c as hydration_mismatch, f as ownership_invalid_binding, i as console_log_state, l as invalid_raw_snippet_render, m as select_multiple_invalid_value, o as hydration_attribute_changed, p as ownership_invalid_mutation, r as binding_property_non_reactive, s as hydration_html_changed, t as assignment_value_stale, y as true_default } from "./warnings-Ds7e8mX2.js";
import { $ as autofocus, $t as previous_batch, A as destroy_effect, An as set_hydrate_node, Ar as EFFECT_TRANSPARENT, B as remove_effect_dom, Bn as HMR, C as untrack, Cn as hydrate_next, Cr as svelte_element_invalid_this_value, Ct as mutable_source, D as block, En as hydrating, Er as DESTROYED, Et as source, Fn as EACH_IS_ANIMATED, Fr as LOADING_ATTR_SYMBOL, Ft as capture, G as template_effect, Gn as NAMESPACE_HTML, Gr as get_prototype_of, H as resume_effect, Hn as HYDRATION_ERROR, Ht as unset_context, In as EACH_IS_CONTROLLED, It as flatten, J as validate_effect, Jn as PROPS_IS_BINDABLE, Jr as is_promise, Jt as current_batch, K as user_effect, Kr as is_array, L as move_effect, Ln as EACH_ITEM_IMMUTABLE, Lr as STATE_SYMBOL, M as effect, Mn as skip_nodes, Mr as HEAD_EFFECT, Mt as derived_safe_equal, N as effect_root, Nn as ATTACHMENT_KEY, Nr as INERT, O as branch, On as read_hydration_instruction, Or as DOCUMENT_FRAGMENT_NODE, Ot as update, Pn as EACH_INDEX_REACTIVE, Pr as LEGACY_PROPS, Q as add_form_reset_listener, Qn as PROPS_IS_UPDATED, Qr as run_all, Qt as is_flushing_sync, R as pause_children, Rn as EACH_ITEM_REACTIVE, S as tick, Sn as safe_not_equal, Sr as store_invalid_shape, St as internal_set, Tr as COMMENT_NODE, Tt as set, U as run_out_transitions, Un as HYDRATION_START, Ur as get_descriptor, V as render_effect, Vn as HYDRATION_END, Vr as define_property, W as teardown, Wn as HYDRATION_START_ELSE, Wr as get_descriptors, Wt as get_boundary, X as listen_to_event_and_reset_event, Xn as PROPS_IS_LAZY_INITIAL, Xr as object_keys, Xt as flushSync, Y as listen, Yn as PROPS_IS_IMMUTABLE, Yr as noop, Z as without_reactive_context, Zn as PROPS_IS_RUNES, Zr as run$1, _r as rune_outside_svelte, an as dev_current_component_function, ar as TRANSITION_OUT, bn as snapshot, br as lifecycle_outside_component, bt as proxy, cr as component_api_changed, ct as create_text, d as active_reaction, dr as get_abort_signal_outside_reaction, dt as get_next_sibling, g as is_destroying_effect, gr as props_rest_readonly, h as get, hn as get_stack, hr as props_invalid_value, ht as should_defer_append, i as delegate, ir as TRANSITION_IN, it as clear_text_content, j as eager_effect, jn as set_hydrating, jr as ELEMENT_NODE, jt as derived, kr as EFFECT_RAN, lr as component_api_invalid_new, mn as set_dev_current_component_function, mr as lifecycle_legacy_only, on as dev_stack, or as UNINITIALIZED, p as deep_read_state, pr as invalid_snippet, q as user_pre_effect, qn as NAMESPACE_SVG, qr as is_function, qt as Batch, r as create_event, rn as component_context, rr as TRANSITION_GLOBAL, sr as bind_invalid_checkbox_value, tn as queue_micro_task, u as active_effect, un as is_runes, ur as each_key_duplicate, ut as get_first_child, v as set_active_effect, wn as hydrate_node, wr as BLOCK_EFFECT, xn as dynamic_void_element_content, xr as snippet_without_render_tag, y as set_active_reaction, yr as invalid_snippet_arguments, yt as is, z as pause_effect, zn as FILENAME, zr as array_from, zt as run_after_blockers } from "./events-CwpACFa3.js";
import { a as legacy_mode_flag } from "./flags-ZndxuwIb.js";
...
function each(node, flags, get_collection, get_key, render_fn, fallback_fn = null) {
	...
	var each_effect;
	function commit() {
		reconcile(each_effect, array, state$1, offscreen_items, anchor, render_fn, flags, get_key, get_collection);
		if (fallback_fn !== null) {
			if (array.length === 0) if (fallback$1) resume_effect(fallback$1);
			else fallback$1 = branch(() => fallback_fn(anchor));
			else if (fallback$1 !== null) pause_effect(fallback$1, () => {
				fallback$1 = null;
			});
		}
	}
	block(() => {
		...
		if (hydrating) {
			...
			for (var i = 0; i < length; i++) {
				if (hydrate_node.nodeType === COMMENT_NODE && hydrate_node.data === HYDRATION_END) {
					anchor = hydrate_node;
					mismatch = true;
					set_hydrating(false);
					break;
				...
				var value = array[i];
				var key$1 = get_key(value, i);
				item = create_item(hydrate_node, state$1, prev, null, value, key$1, i, render_fn, flags, get_collection);
				state$1.items.set(key$1, item);
				prev = item;
			}
			if (length > 0) set_hydrate_node(skip_nodes());
		}
		if (hydrating) {
			if (length === 0 && fallback_fn) fallback$1 = branch(() => fallback_fn(anchor));
			...
			for (i = 0; i < length; i += 1) {
				...
				var existing = state$1.items.get(key$1) ?? offscreen_items.get(key$1);
				if (existing) {
					if ((flags & (EACH_ITEM_REACTIVE | EACH_INDEX_REACTIVE)) !== 0) update_item(existing, value, i, flags);
				} else {
					item = create_item(null, state$1, null, null, value, key$1, i, render_fn, flags, get_collection, true);
					offscreen_items.set(key$1, item);
				}
				keys.add(key$1);
			}
			for (const [key$2, item$1] of state$1.items) if (!keys.has(key$2)) batch.skipped_effects.add(item$1.e);
...
* @returns {void}
*/
function reconcile(each_effect, array, state$1, offscreen_items, anchor, render_fn, flags, get_key, get_collection) {
	...
	for (i = 0; i < length; i += 1) {
		...
		item = items.get(key$1);
		if (item === void 0) {
			var pending$2 = offscreen_items.get(key$1);
			if (pending$2 !== void 0) {
				offscreen_items.delete(key$1);
				items.set(key$1, pending$2);
				var next$1 = prev ? prev.next : current;
				link(state$1, prev, pending$2);
				link(state$1, pending$2, next$1);
				move(pending$2, next$1, anchor);
				prev = pending$2;
			} else prev = create_item(current ? current.e.nodes_start : anchor, state$1, prev, prev === null ? state$1.first : prev.next, value, key$1, i, render_fn, flags, get_collection);
			items.set(key$1, prev);
			matched = [];
			stashed = [];
			current = prev.next;
			continue;
		...
		if (item !== current) {
			...
			matched = [];
			stashed = [];
			while (current !== null && current.k !== key$1) {
...
function update_item(item, value, index$1, type) {
	if ((type & EACH_ITEM_REACTIVE) !== 0) internal_set(item.v, value);
	if ((type & EACH_INDEX_REACTIVE) !== 0) internal_set(item.i, index$1);
	else item.i = index$1;
}
/**
* @template V
...
* @returns {EachItem}
*/
function create_item(anchor, state$1, prev, next$1, value, key$1, index$1, render_fn, flags, get_collection, deferred) {
	var previous_each_item = current_each_item;
	var reactive = (flags & EACH_ITEM_REACTIVE) !== 0;
	var mutable = (flags & EACH_ITEM_IMMUTABLE) === 0;
	var v = reactive ? mutable ? /* @__PURE__ */ mutable_source(value, false, false) : source(value) : value;
	var i = (flags & EACH_INDEX_REACTIVE) === 0 ? index$1 : source(index$1);
 ...
 /** @type {Value} */ v.trace = () => {
	...
	try {
		...
		if (prev === null) {
			if (!deferred) state$1.first = item;
		} else {
			prev.next = item;
			prev.e.next = item.e;
		}
	...
	block(() => {
		...
		branches.ensure(next_tag, (anchor$1) => {
			...
			if (next_tag) {
				...
				/** @type {Effect} */ active_effect.nodes_end = element$1;
...
function transition(flags, element$1, get_fn, get_params) {
	...
	*/
	...
	var e = active_effect;
	(e.transitions ??= []).push(transition$1);
...
//#endregion
export { bind_ready_state as $, validate_snippet_args as $t, writable as A, clsx as At, bind_property as B, prevent_snippet_stringification as Bt, store_unsub as C, set_xlink_attribute as Ct, get$1 as D, set_style as Dt, derived$1 as E, select_option as Et, init as F, element as Ft, bind_online as G, slot as Gt, bind_element_size as H, validate_store as Ht, bind_window_scroll as I, component as It, bind_ended as J, index as Jt, bind_buffered as K, html as Kt, bind_window_size as L, createRawSnippet as Lt, bubble_event as M, action as Mt, reactive_import as N, append_styles as Nt, readable as O, set_class as Ot, update_legacy_props as P, head as Pt, bind_played as Q, await_block as Qt, bind_content_editable as R, snippet as Rt, store_set as S, set_value as St, update_store as T, init_select as Tt, bind_resize_observer as U, validate_void_dynamic_element as Ut, bind_this as V, validate_dynamic_element_tag as Vt, bind_prop as W, sanitize_slots as Wt, bind_paused as X, key as Xt, bind_muted as Y, css_props as Yt, bind_playback_rate as Z, if_block as Zt, invalidate_store as _, set_checked as _t, onDestroy as a, hmr as an, bind_group as at, store_get as b, set_default_value as bt, create_custom_element as c, assign as cn, animation as ct, legacy_rest_props as d, assign_or as dn, raf as dt, async as en, bind_seekable as et, prop as f, createAttachmentKey as fn, CLASS as ft, update_prop as g, set_attribute as gt, update_pre_prop as h, remove_input_defaults as ht, getAbortSignal as i, create_ownership_validator as in, bind_files as it, add_legacy_event_listener as j, attach as jt, readonly as k, attr as kt, validate_binding as l, assign_and as ln, transition as lt, spread_props as m, attribute_effect as mt, beforeUpdate as n, check_target as nn, bind_volume as nt, onMount as o, add_locations as on, bind_value as ot, rest_props as p, fromAction as pn, STYLE as pt, bind_current_time as q, each as qt, createEventDispatcher as r, legacy_api as rn, bind_checked as rt, log_if_contains_state as s, cleanup_styles as sn, bind_active_element as st, afterUpdate as t, inspect as tn, bind_seeking as tt, validate_each_keys as u, assign_nullish as un, loop as ut, mark_store_binding as v, set_custom_element_data as vt, update_pre_store as w, bind_select_value as wt, store_mutate as x, set_selected as xt, setup_stores as y, set_default_checked as yt, bind_focused as z, wrap_snippet as zt };
//# sourceMappingURL=index-client-CBge2tFO.js.map